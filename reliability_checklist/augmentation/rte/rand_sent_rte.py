...
class rand_sentence_augmentation_rte:
    def __init__(self):
    ...
    def infer(self, dataset, n_workers="max"):
        ...
        for i in tqdm(range(len(dataset))):
            if cryptogen.uniform(0, 1) < 0.5:
                new_hypothesis = (
                    cryptogen.choice(self.IRRELEVANT_SENTENCES) + " " + dataset["sentence1"][i]
                )
            else:
                new_hypothesis = (
                    dataset["sentence1"][i] + " " + cryptogen.choice(self.IRRELEVANT_SENTENCES)
                )
            new_dataset["sentence1"].append(new_hypothesis)
