    ...
    def infer(self, dataset, n_workers="max"):
        ...
        for i in tqdm(range(len(dataset))):
            if(random.uniform(0, 1) < 0.5): 
                new_hypothesis = random.choice(self.IRRELEVANT_SENTENCES) + " " + dataset["hypothesis"][i] 
            else:
                new_hypothesis = dataset["hypothesis"][i] + " " + random.choice(self.IRRELEVANT_SENTENCES)
